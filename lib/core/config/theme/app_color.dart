import 'package:flutter/material.dart';

class AppColor {
  static const primaryColor = Color(0xFF40D876);
  static const backgroundColor = Color(0xFF131429);
  static const scoundColor = Color.fromARGB(255, 15, 84, 157);
  static const thredColor = Color(0xFF232441);
  static const wColor = Colors.white;
  static const redColor = Colors.red;
  static const bColor = Colors.black;
  static const yColor = Color.fromARGB(255, 225, 235, 94);
  static Color grayColor = Colors.grey[300]!;
  static Color gColor = Colors.grey;
  static Color noColor = Colors.transparent;
  static Color backGroundListGenerate = const Color(0xFF3a3d46);
  static Color subTitleTextColorInGoalScreen = const Color(0xFF76777f);
  static Color textColor = const Color(0xFFd0d2d2);
  static Color profileBackgroundColor = const Color(0xFFF3F3F3);
  static Color profileItemColor = const Color(0xFFFFFFFF);


}
